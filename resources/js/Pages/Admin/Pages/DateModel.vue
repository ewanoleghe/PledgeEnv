...
const updatePaymentStatusC = async (id) => {
    try {
        // Send a PATCH request to update the payment status while preserving scroll
        await router.post(route('admin.calendar.update_cal_payment_status'), { id }, { preserveScroll: true });
            ...
            <div class="flex flex-col md:flex-row justify-center space-x-0 md:space-x-4 h-full w-full"> <!-- Added w-full here -->
                <div class="w-full flex flex-col justify-between mb-4 md:mb-0">
                    <div class="p-1 flex-grow">
                        <h2 class="text-xl font-bold p-1">All Lead Booking  {{ bookings.selected_date }}</h2>
                        ...
                        <div class="overflow-x-auto p-2">
                         ...
                         <!-- Table Goes Here -->
                                <table class="min-w-full border-collapse">
                                    ...
                                    <tbody>
                                        <!-- <tr v-for="calData in bookings.data" :key="calData.id"> -->
                                            
                                            <tr 
                                            v-for="calData in bookings.data" 
                                                :key="calData.id"
                                                :class="{
                                                    'bg-red-300': calData.jobStatus === 'failed', 
                                                    'bg-indigo-100': calData.payment_status === 'pending'
                                            }">
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900 font-bold">{{ calData.order_id }}</span><br>
                                                <span v-if="calData.old_order_id" class="text-[0.7rem] text-black-900">({{ calData.old_order_id }})</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900 font-bold">
                                                    {{ calData.methodology }}<span v-if="calData.includeXrf"><br> + XRF</span>
                                                </span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900 font-bold">{{ calData.municipality }}</span><br>
                                                <span class="text-xs text-black-900">{{ calData.county }}</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs font-bold text-black-900">{{ calData.address }}</span><br>
                                                <span class="text-xs text-black-900">Zip: {{ calData.city }}</span>
                                            </td>
                                            <td class="text-black justify-center border border-black pl-2">
                                                <span class="text-xs text-black-900 font-bold">{{ calData.selected_date }}</span><br>
                                                <span class="text-xs text-black-900 font-bold">{{ calData.selectedTime == 'morning' ? 'Morning' : 'Afternoon' }}</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900 font-bold">{{ calData.name }}</span><br>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900">{{ calData.email }}</span><br>
                                                <span class="text-xs text-black-900">Ph: {{ calData.phone }}</span>
                                            </td>
                                            <td class="text-black text-center border border-black">
                                                <span class="text-xs text-black-900">{{ calData.assignTo }} 
                                                    <sup v-if="calData.jobStatus === 'completed'" class="inline-block w-2 h-2 bg-green-500 rounded-full ml-1"></sup>
                                                </span>
                                            </td>
                                            <td 
                                                :class="[calData.payment_status === 'succeeded' ? 'bg-white' : 'bg-blue-300', 
                                                        'text-black', 'text-center', 'font-bold', 'text-xs', 'border', 'p-2', 'border-black']">
                                                <button 
                                                    @click="updatePaymentStatusC(calData.id)"
                                                    class="text-black">
                                                    {{ calData.payment_status === 'succeeded' ? 'Set pending' : 'Set Paid' }}
                                                </button>
                                            </td>
